...
public class AutoCollectorImpl extends AbstractService {
    ...
    protected void makeTurn() {
        ...
        if (battleService.getItems().size() == 0) {
            return;
        }
        for(Item item: battleService.getItems()) {
        ...
        int px = battleService.getPlayer().getX();
        int py = battleService.getPlayer().getY();
        Direction direction = battleService.getPlayer().getDirection();
        List<BattleAction> actions = new ArrayList<BattleAction>();
        int cmove = 0;
        // 2 6 12 20
        int[] moves = new int[]{2, 4, 6, 8, 10};
        int od = 20;
        aloop:
        while (true) {
            int current = m[px][py];
